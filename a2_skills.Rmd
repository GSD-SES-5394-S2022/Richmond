...
* **Create a tree map:** There is a school of thought that is really against pie charts, and the developers of ggplot are part of that school, so they make it tricky to make a pie chart. Tree maps are nice alternative that avoid some of the pitfalls of pie charts. 
...
For each variable I want to change, I'll use an `ifelse()` function to indicate that for a row where the GEOID is "36029004602" (`GEOID == "36029004602"` in the first argument of the `ifelse()` function), I want to increase the value by 100 (`total_hhsE + 100` in the second argument), otherwise I'll leave the original value (`total_hhsE` in the last argument).

```{r, eval=FALSE}
library(tidyverse)

zones_new <- final_data %>%
  mutate(total_hhsE = ifelse(GEOID == "36029004602",
                             total_hhsE + 100,
                             total_hhsE),
         no_vehE = ifelse(GEOID == "36029004602",
                             no_vehE + 100,
                             no_vehE),
         hh_1personE = ifelse(GEOID == "36029004602",
                             hh_1personE + 100,
                             hh_1personE),
         inc_quint_1 = ifelse(GEOID == "36029004602",
                             inc_quint_1 + 100,
                             inc_quint_1))
```
...
There is a school of thought that is really against pie charts, and the developers of ggplot are part of that school, so they make it tricky to make a pie chart. You can still do it though because a pie chart is secretly just a stacked bar chart in a polar coordinate system.

First I'll need to get some total values for the whole region (see the topic on "Get regional data").
...
I could create a stacked bar chart to show those percentages.

```{r}
bar_chart <- ggplot(region_vehs, aes(x = 1, 
                                     y = pct, 
                                     fill = variable,
...
I can also wrap that bar chart around a polar coordinate system to make a pie chart.
...
There is a school of thought that is really against pie charts, and the developers of ggplot are part of that school, so they make it tricky to make a pie chart. Tree maps are nice alternative that avoid some of the pitfalls of pie charts. 
...
If you also want to get region-level data in addition to just the boundaries, you can use the tidycensus package (see the "Get Regional Data" topic).
...
If your study area has a lot of counties, you might find it really tiresome to type in the names of all of them. Another approach is to define the study area boundary (see the "Define study area boundary" topic - in this example, I've saved it to an layer called `boundary`), then download census tracts for the entire state (i.e. leave the county argument out), and then filter them to only include the ones within the study area. This will not work if you do not set `geometry = TRUE` in the `get_acs()` function.

In order to avoid capturing that are adjacent to my study area, I'm going to select based on centroid locations.

```{r, eval=FALSE}
library(tidycensus)
library(sf)

va_census <- get_acs(geography = 'tract',
                 state = 'VA',
                 variables = vars,
                 output = 'wide',
                 geometry = TRUE) 

census_centroids <- st_centroid(va_census) %>%
  st_filter(boundary)

census <- va_census %>%
  st_filter(census_centroids)
```
...
First I'll create a list of variables indicating the number of households in each of 16 income categories (see the "List census variable names" topic).
...
Then I'll download those variables (see the "Download Census tract data" topic)  for all the core-based statistical areas in United States (`geography = metropolitan statistical area/micropolitan statistical area"` instead of `geography = "tract"`). I am *not* setting `output = 'wide'` because in this case I actually *do* want each variable in its own row (there will be a column with the variable name and column called `estimate` with the value of each variable). I'm setting `summary_var = 'B19001_001'` because that's the total number of households, and I want to calculate what percent of total households fall in each category.

If you need the spatial extent of the study area, you could just add `geometry = TRUE` to the `get_acs()` function instead of [getting the boundaries through the `tigris` package] (see the "Define study area boundary" topic for how to get the boundary from the Tigris package). I'm setting `geometry = FALSE` in this case, assuming I don't need the study area boundary data.

After a pipe (`%>%`) I filter the data to only include my study area (I'm using Buffalo in this example - the GEOID is 15380).

After another pipe, I create a variable (see the "Calculate new variables" topic) with the percent of households in each category, and then I use the `cumsum()` function to calculate a cumulative percentage. Finally, I delete the columns I don't need (see the "Organize data columns" topic), keeping only the cumulative percentage and variable names.
...
Now I can print the table (see the "Summarize tract statistics" topic).

```{r}
kable(region_inc, digits=2)
```

From the table above, I can see that 20 percent of households have incomes below \$25,000 per year, 40 percent have incomes below \$50,000 per year, 60 percent (or close enough) have incomes below \$75,000, and 80 percent have incomes below \$100,000 per year. These are thresholds I could use to define broader income categories for my tract-level data. 
...
Here are the first few rows of a dataframe with a 59 columns:
